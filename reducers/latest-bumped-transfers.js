...
export default (
  ...
  switch (action.type) {
    case LATEST_BUMPED_TRANSFERS_DATA:
      ...
      latest_bumped_transfers_data = _.uniqBy(
        toArray(
          _.concat(
            latest_bumped_transfers_data,
            toJson(action.value) || { transfer_id: action.value, updated: moment().valueOf() },
          )
        ).filter(d => moment().diff(moment(d.updated), 'minutes', true) <= 5),
        'transfer_id',
      )
