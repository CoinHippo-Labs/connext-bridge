              ...
              columns={[
                {
                  ...
                  Cell: props => {
                    ...
                    const { id, pools, chain_data, asset_data, contract_data } = { ...row.original }
                    let { name } = { ...row.original }
                    const { symbol, image } = { ...value }
                      ...
                      <div className="flex flex-col space-y-3">
                        {view === 'my_positions' ?
                          ...
                          <>
                            ...
                            {uncollapseAssetIds?.includes(id) && toArray(pools).map((p, i) => {
                              const { chain_data, asset_data, contract_data } = { ...p }
                              let { name } = { ...p }
                              ...
                              const asset = asset_data?.id
                              const component = (
                                <div className="h-6 3xl:h-12 flex items-center 3xl:text-2xl font-medium ml-8">
                                  {name}
                                </div>
                              )
                ...
                },
                {
                  ...
                  Cell: props => {
                    const { value, row } = { ...props }
                    const { id, pools, chain_data, asset_data } = { ...row.original }
                    ...
                    const onClick = () => {
                      if (pools?.length > 0) {
                        setUncollapseAssetIds(uncollapseAssetIds?.includes(id) ? uncollapseAssetIds.filter(_id => _id !== id) : _.concat(toArray(uncollapseAssetIds), id))
                      }
                    }
                                  ...
                                  <div className="mr-1.5">
                                    ...
                                    >
                                      {uncollapseAssetIds?.includes(id) ?
                                        <MdKeyboardArrowUp size={16} className="3xl:w-5 3xl:h-5" /> :
                                        <MdKeyboardArrowDown size={16} className="3xl:w-5 3xl:h-5" />
                                      }
                                    </button>
                                  </div>
                                </> :
                  ...
                  Cell: props => {
                    ...
                    return (
                      <div className="flex flex-col space-y-3">
                        ...
                        {uncollapseAssetIds?.includes(id) && toArray(pools).map((p, i) => {
                          const { chain_data, asset_data, tvl, lpTokenAddress, error } = { ...p }
                          const chain = chain_data?.id
                          const asset = asset_data?.id
                          const value = tvl
                          const component = (
                  ...
                  Cell: props => {
                    ...
                    return (
                      <div className="flex flex-col space-y-3">
                        ...
                        {uncollapseAssetIds?.includes(id) && toArray(pools).map((p, i) => {
                          const { chain_data, asset_data, volume_value } = { ...p }
                          const chain = chain_data?.id
                          const asset = asset_data?.id
                  ...
                  Cell: props => {
                    ...
                    return (
                      <div className="flex flex-col space-y-3">
                        ...
                        {uncollapseAssetIds?.includes(id) && toArray(pools).map((p, i) => {
                          const { chain_data, asset_data, fees_value } = { ...p }
                          const chain = chain_data?.id
                          const asset = asset_data?.id
                ...
                },
                {
                  ...
                  Cell: props => {
                    const { value, row } = { ...props }
                    const { chain_data, asset_data, symbol } = { ...row.original }
                    const chain = chain_data?.id
                    const asset = asset_data?.id
                    const component = (
                      <div className="h-6 3xl:h-12 flex flex-col justify-center space-y-1">
                ...
                },
                {
                  ...
                  Cell: props => {
                    const { row } = { ...props }
                    const { chain_data, asset_data, adopted, local, lpTokenBalance, price } = { ...row.original }
                    const chain = chain_data?.id
                    const asset = asset_data?.id
                    const component = (
                      <div className="h-6 3xl:h-12 flex items-center justify-end space-x-1.5">
                        <DecimalsFormat
                        ...
                        {/*
                          ...
                          <span className="text-sm 3xl:text-2xl font-medium">/</span>
                          <div className="flex items-center text-sm 3xl:text-2xl font-medium space-x-1">
                            <DecimalsFormat
                              value={local?.balance}
                              className="uppercase 3xl:text-2xl"
