...
const menus = toArray([
  {
    title: 'Developers',
    icon: <HiCode size={20} className="3xl:w-6 3xl:h-6 text-blue-500 dark:text-white" />,
    items: toArray([
      process.env.NEXT_PUBLIC_DOC_URL && {
        title: 'Documentation',
        url: process.env.NEXT_PUBLIC_DOC_URL,
        icon: <CgFileDocument size={20} className="3xl:w-6 3xl:h-6 text-blue-500 dark:text-white" />,
      },
      process.env.NEXT_PUBLIC_CONNEXT_GITHUB_URL && {
        title: 'Connext GitHub',
        url: process.env.NEXT_PUBLIC_CONNEXT_GITHUB_URL,
        icon: <BsGithub size={20} className="3xl:w-6 3xl:h-6 text-black dark:text-white" />,
      },
  ...
  process.env.NEXT_PUBLIC_TELEGRAM_USERNAME && {
    ...
    icon: <BsTelegram size={20} className="3xl:w-6 3xl:h-6 text-blue-500 dark:text-white" />,
  },
  process.env.NEXT_PUBLIC_DISCORD_URL && {
    title: 'Discord',
    url: process.env.NEXT_PUBLIC_DISCORD_URL,
    icon: <FaDiscord size={20} className="3xl:w-6 3xl:h-6 text-blue-600 dark:text-white" />,
  },
])
  ...
  return (
    <div className="flex flex-wrap pb-0">
      {openMenu ?
        ...
        toArray(menus).map((d, i) => {
          const { title, url, icon, items } = { ...d }
          const item = (
            <>
              {icon}
              <span className="3xl:text-xl font-semibold">
