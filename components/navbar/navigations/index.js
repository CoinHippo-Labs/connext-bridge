...
export default () => {
  ...
  const { bridge, swap, source } = { ...query }

  return (
    <div className="hidden lg:flex items-center space-x-0 xl:space-x-2 3xl:space-x-3 mx-auto">
      {menus.filter(m => m.path && !['pool'].includes(source)) .map(m => {
        const { id, disabled, others_paths, external, icon } = { ...m }
        let { title, path } = { ...m }
        ...
        switch (id) {
          ...
          case 'swap':
            if (pathname === '/swap/[swap]' && swap) {
              ...
              let asset
              if (chains_data && assets_data) {
                const _chains_data = getChain(chain_id, chains_data, false, true, false, undefined, true).filter(c => getAsset(null, assets_data, c?.chain_id, undefined, undefined, true, true, true))
        ...
        const className = `bg-transparent hover:bg-blue-50 dark:hover:bg-slate-800 rounded ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'} flex items-center uppercase ${selected ? 'text-blue-600 dark:text-white text-sm font-extrabold' : 'text-slate-600 hover:text-slate-700 dark:text-slate-300 dark:hover:text-slate-200 text-sm font-semibold hover:font-bold'} space-x-1.5 py-2 px-2.5 3xl:text-xl 3xl:space-x-3`
        return (
          external ?
            <a
              key={id}
