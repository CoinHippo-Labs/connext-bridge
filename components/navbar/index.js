...
export default () => {
  ...
  const { theme, page_visible, status_message } = { ...preferences }
  ...
  const { default_chain_id, chain_id, provider, ethereum_provider, signer, address } = { ...wallet_data }
  ...
  useEffect(
    () => {
      ...
      if (status_message) {
        dispatch({ type: STATUS_MESSAGE, value: status_message })
      }
    },
  ...
  useEffect(
    () => {
      const getData = async () => {
        dispatch({ type: CHAINS_DATA, value: toArray(await getChains()) })
      }
      getData()
    },
    [],
  ...
  useEffect(
    () => {
      const getData = async () => {
        ...
        dispatch({ type: ASSETS_DATA, value: response })
        dispatch({ type: POOL_ASSETS_DATA, value: getAsset(null, response, undefined, undefined, undefined, true, false, true, true) })
      }
      getData()
    },
    [],
      ...
      const getData = async is_interval => {
        if (page_visible && assets_data) {
          ...
          if (updated_ids.length < assets_data.length) {
            ...
            if (updated) {
              dispatch({ type: ASSETS_DATA, value: _.cloneDeep(assets_data) })
            }
          }
          ...
          if (pool_assets_data.findIndex(d => !updated_ids.includes(d?.id)) < 0) {
            dispatch({ type: POOL_ASSETS_DATA, value: pool_assets_data })
          }
        }
      }
  ...
  useEffect(
    () => {
      const createRpcProvider = (url, chain_id) => new providers.StaticJsonRpcProvider(url, chain_id ? Number(chain_id) : undefined)
      const init = async => {
        if (chains_data) {
          const _rpcs = {}
          for (const chain_data of chains_data) {
            const { disabled, chain_id, provider_params } = { ...chain_data }
            let { rpc_urls } = { ...chain_data }
            if (!disabled) {
              rpc_urls = /*rpc_urls || */toArray(_.head(provider_params)?.rpcUrls)
              _rpcs[chain_id] = rpc_urls.length > 1 ?
          ...
          if (!rpcs) {
            dispatch({ type: RPCS, value: _rpcs })
          }
        }
      }
      init()
    ...
    () => {
      const init = async () => {
        if (!sdk && chains_data && assets_data && assets_data.findIndex(a => typeof a.price !== 'number') < 0) {
          const chains = {}
          for (const chain_data of chains_data) {
            const { disabled, chain_id, domain_id, provider_params } = { ...chain_data }
            let { rpc_urls } = { ...chain_data }
            if (!disabled && domain_id) {
              rpc_urls = rpc_urls || toArray(_.head(provider_params)?.rpcUrls)
              chains[domain_id] = {
                providers: rpc_urls,
                assets: getAsset(null, assets_data, chain_id, undefined, undefined, true, false, false, true).map(a => {
                  const { contracts } = { ...a }
                  let { name, symbol } = { ...a }
          ...
          try {
            ...
            const data = _.groupBy(
              response.map(d => {
                ...
                const asset_data = getAsset(null, assets_data, chain_id, undefined, undefined, false, false, false, true).find(a => getContract(adopted, toArray(a?.contracts), chain_id) || getContract(local, toArray(a?.contracts), chain_id))
                return {
                  ...d,
                  chain_id,
                  chain_data,
    ...
    () => {
      const getData = async () => {
        if (
          ...
          if (_ens_data) {
            dispatch({ type: ENS_DATA, value: _ens_data })
          }
        }
      }
      getData()
  ...
  useEffect(
    () => {
      const getPoolData = async (chain_data, asset_data) => {
        ...
        if (contract_address) {
          ...
          try {
            ...
            let { price } = { ...getAsset(asset_data.id, assets_data) }
            price = price || 0
            ...
            data = getPool(id, pools_data) || { id, chain_id, chain_data, asset_data, contract_data, error }
          }
          if (data) {
    ...
    () => {
      const getData = async is_interval => {
        if ([/*'/pools', */'/pool/[pool]'].includes(pathname) && (is_interval || !pools_daily_stats_data)) {
          ...
          dispatch({ type: POOLS_DAILY_STATS_DATA, value: { tvls, volumes } })
        }
      }
  ...
  useEffect(
    () => {
      const getMyBalance = async (chain_id, contract_data) => {
        const { contract_address, next_asset, wrapable } = { ...contract_data }
        const provider = rpcs[chain_id]
        if (provider) {
          const contracts_data = toArray(
            _.concat(
              { ...contract_data },
              wrapable && { ...contract_data, contract_address: constants.AddressZero },
      ...
      const getData = async is_interval => {
        if (page_visible && chains_data && assets_data && assets_data.findIndex(a => typeof a.price !== 'number') < 0 && rpcs && address) {
          const all_chains_data = getChain(null, chains_data, false, false, false, undefined, true)
          const data = get_balances_data && !is_interval && all_chains_data.findIndex(c => !balances_data?.[c.chain_id]) < 0 ?
      ...
      <div className="navbar 3xl:pt-6">
        <div className="navbar-inner w-full sm:h-20 flex xl:grid xl:grid-flow-row xl:grid-cols-3 items-center justify-between gap-4">
          ...
          <div className="flex items-center justify-end 3xl:space-x-4">
            {provider && <Chains chain_id={chain_id} />}
            {provider && address && (
