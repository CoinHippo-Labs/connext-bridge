...
export default ({ useModal = false }) => {
  const dispatch = useDispatch()
  const { preferences } = useSelector(state => ({ preferences: state.preferences }), shallowEqual)
  const { terms_agreed } = { ...preferences }
      ...
      <div className="space-y-7">
        <div className="space-y-3.5">
          <div className="flex items-start space-x-3.5">
            <IoKeyOutline size={18} className="text-slate-400 dark:text-slate-500 mt-0.5" style={{ minWidth: '18px' }} />
          ...
          <div className="flex items-start space-x-3.5">
            <IoDocumentTextOutline size={18} className="text-slate-400 dark:text-slate-500 mt-0.5" style={{ minWidth: '18px' }} />
            <div className="flex-wrap leading-5 text-sm 3xl:text-base font-semibold">
              ...
              <a
                href={`${process.env.NEXT_PUBLIC_DOC_URL}/concepts/what-is-connext`}
                target="_blank"
                rel="noopener noreferrer"
  ...
  return terms_agreed === false && (
    useModal ?
      ...
      /> :
      <div className="max-w-2md 3xl:max-w-xl bg-slate-100 dark:bg-slate-800 rounded mt-0.5 mx-auto py-6 px-7">
        {title}
        {body}
      </div>
  )
