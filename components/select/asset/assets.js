...
export default (
  ...
  const _assets_data =
    (isPool ?
      toArray(
        _.concat(
          pool_assets_data,
          data && toArray(pools_data).filter(p => equalsIgnoreCase(p?.domainId, domain_id)).map(p => {
            const { asset_data, contract_data, adopted, local } = { ...p }
  ...
  const assets_data_sorted = _.orderBy(
    toArray(_assets_data)
      ...
      .flatMap(a => {
        const { symbol, image, contracts } = { ...a }
        const contract_data = getContract(chain_id, contracts)
        const { next_asset, wrapable } = { ...contract_data }
        const contracts_data = toArray(
          _.concat(
            ...
            (!showOnlyWrapable || wrapable) && { ...contract_data },
        ...
        return (
          contracts_data.map(c => {
            const { is_next_asset } = { ...c }
            const _contracts = _.cloneDeep(contracts).map(_c => getContract(chain_id, contracts) ? c : _c)
            return {
              ...a,
              is_next_asset,
              contracts: _contracts,
      ...
      .map(a => {
        const { is_next_asset, group, scores } = { ...a }
        return {
          ...a,
    ...
    <div>
      {preset_assets_data.length > 0 && !showOnlyWrapable && (
        <div className="flex flex-wrap items-center mt-1 mb-4">
          {preset_assets_data.map((a, i) => (
            <div
              key={i}
              onClick={
                () => {
                  const { id, contracts } = { ...a }
                  const contract_data = getContract(chain_id, contracts, chain_id, false, true).find(c => c.wrapable || c.contract_address)
                  const { wrapable } = { ...contract_data }
      ...
      <div className="max-h-96 overflow-y-scroll">
        {assets_data_sorted.map((a, i) => {
          const { id, disabled, contracts, group } = { ...a }
          const contract_data = getContract(chain_id, contracts)
          ...
          let { amount } = { ...getBalance(chain_id, contract_address, balances_data) }
          amount = ['string', 'number'].includes(typeof amount) && !isNaN(amount) ? amount : null
