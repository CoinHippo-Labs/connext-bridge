  ...
  while (hasMore) {
    ...
    data = _.uniqBy(_.concat(data || [], response?.data?.transactions?.map(transaction => {
      return {
        ...
        sendingAsset: contracts?.[`${transaction.sendingChainId}_${transaction.sendingAssetId}`],
        ...
        receivingAsset: contracts?.[`${transaction.receivingChainId}_${transaction.receivingAssetId}`],
      }
    }).map(transaction => {
      return {
        ...transaction,
...
export const transactionFromSdk = async (sdk, chain_id, tx_id, chains, contracts) => {
  ...
  if (sdk && tx_id) {
    ...
    data = _.uniqBy(_.concat(data || [], response?.transactions?.map(transaction => {
      return {
        ...transaction,
        chainTx: getChainTx(transaction),
        chainId: Number(transaction.chainId),
