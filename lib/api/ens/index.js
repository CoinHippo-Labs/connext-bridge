...
export const getENS = async addresses => {
  if (addresses) {
    ...
    for (const _addresses of addresses_chunk) {
      const response = await domains({ where: `{ resolvedAddress_in: [${_addresses.map(a => `"${a}"`).join(',')}] }` })
      const { data } = { ...response }
    ...
    if (domains_data?.length > 0) {
      ...
      for (const address of addresses) {
        const resolved_addresses = domains_data.filter(d => equalsIgnoreCase(d.resolvedAddress?.id, address))
      ...
      return (
        Object.fromEntries(
          domains_data
            .filter(d => !ens_data?.[d.resolvedAddress?.id?.toLowerCase()]?.reverseRecord || equalsIgnoreCase(d.name, ens_data[d.resolvedAddress.id.toLowerCase()].reverseRecord))
            .map(d => [d?.resolvedAddress?.id?.toLowerCase(), { ...d }])
        )
      )
    }
  }
...
export const domainFromENS = async (
  ...
  if (ens) {
    domain = toArray(Object.values({ ...ens_data })).find(d => equalsIgnoreCase(d.name, ens))

    if (!domain) {
      const response = await domains({ where: `{ name_in: ["${ens.toLowerCase()}"] }` })
      const { data } = { ...response }
      domain = toArray(data).find(d => equalsIgnoreCase(d.name, ens))
    }
  }
