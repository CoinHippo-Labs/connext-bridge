  ...
  if (addresses?.length > 0) {
    ...
    if (domains_data?.length > 0) {
      ...
      for (let i = 0; i < addresses.length; i++) {
        ...
        const resolved_addresses = domains_data.filter(d => d?.resolvedAddress?.id?.toLowerCase() === address)
...
export const domainFromEns = async (ens, ens_data = {}) => {
  let domain
  if (ens) {
    domain = Object.values({ ...ens_data }).find(d => d?.name?.toLowerCase() === ens.toLowerCase())
    if (!domain) {
