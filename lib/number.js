...
import { split, toDecimal, numberToFixed } from './utils'
...
export const toFixedNumber = number => FixedNumber.fromString(toDecimal(number)?.toString().includes('.') ? toDecimal(number).toString() : toBigNumber(number))
...
export const parseUnits = (number = '0', decimals = 18) => {
  ...
  try {
    number = numberToFixed(number || 0, decimals)
  } catch (error) {}
