...
export const getAsset = (
  ...
  if ((id || toArray(chainIds).length > 0 || toArray(contractAddresses).length > 0 || toArray(symbols).length > 0 || notDisabled || getHead || onlyPoolAsset || returnAll) && Array.isArray(data)) {
    ...
    data = data.filter(d => (!notDisabled || !d?.disabled) && toArray(chainIds).findIndex(chain_id => toArray(d?.contracts).findIndex(c => c?.chain_id === chain_id) < 0) < 0)
    data = data.filter(d => toArray(d?.contracts).findIndex(c => !onlyPoolAsset || c?.is_pool) > -1)

    const matching_fields = d => [d?.id, d?.symbol]
    const filter = d => matching_fields(d).findIndex(f => typeof f === 'string' && typeof id === 'string' ? equalsIgnoreCase(f, id) : f === id) > -1 || (returnAll && !id)
    const filter_addresses = d => toArray(d?.contracts).findIndex(c =>
      toArray(chainIds).findIndex(chain_id => c?.chain_id === chain_id) > -1 &&
      toArray(contractAddresses)
        .findIndex(a =>
          toArray([c?.contract_address, c?.next_asset?.contract_address]).findIndex(_a => equalsIgnoreCase(_a, a)) > -1
        ) > -1
    ) > -1
    const filter_symbols = d => toArray(d?.contracts).findIndex(c =>
      toArray(chainIds).findIndex(chain_id => c?.chain_id === chain_id) > -1 &&
      toArray(symbols)
        .findIndex(s =>
          toArray([c?.symbol, c?.next_asset?.symbol, d?.symbol]).findIndex(_s => equalsIgnoreCase(_s, s)) > -1
        ) > -1
    ) > -1
...
export const getChainContracts = (chainId, data) => {
  let contracts_data
