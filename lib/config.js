...
export const NUM_STATS_DAYS = 90
export const DEFAULT_SOURCE_CHAIN = NETWORK === 'testnet' ? 'polygon' : 'ethereum'
export const DEFAULT_DESTINATION_CHAIN = NETWORK === 'testnet' ? 'x1' : 'arbitrum'
export const MIN_USER_DEPOSITED = 0.000001

export const getChainsData = () => toArray(NETWORK === 'testnet' ? testnetChains : mainnetChains).filter(c => (!c.is_staging || IS_STAGING) && (!IS_STAGING || !c.no_staging)).map(c => {
...
export const getAssetsData = () => toArray(NETWORK === 'testnet' ? testnetAssets : mainnetAssets).filter(a => (!a.is_staging || IS_STAGING) && (!IS_STAGING || !a.no_staging)).map(a => {
  const { contracts } = { ...a }
  return {
    ...a,
    contracts: toArray(contracts).filter(c => !c.disabled && (!c.is_staging || IS_STAGING)),
  }
})